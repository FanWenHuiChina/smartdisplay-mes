...
const requiredApiExports = [
  ...
  ['ingestQmsInspection', '/v1/adapters/qms/inspections'],
  ['checkWmsMaterialReadiness', '/v1/adapters/wms/material-readiness'],
  ['ingestWmsInventoryTransaction', '/v1/adapters/wms/inventory-transactions'],
...
check('page:views/material/index.vue:wms-location-task-disposition', hasAll(materialView, ['dispositionMaterialLocationTask', 'dispositionLocationTask', 'canDisposition', 'dispositionStatus', 'dispositionResult', 'ACCEPT_DEVIATION', 'ADJUST_INVENTORY', '接收差异', '调库']), 'Material page must expose WMS rejected review disposition and explicit inventory adjustment action')
...
check('layout:legacy-dashboard-page-removed', !existsSync(join(projectRoot, 'src/views/dashboard/index.vue')), 'Unused legacy dashboard page must not keep static Lot/OEE mock data')
